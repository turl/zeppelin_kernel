...
static int msm_set_vreg_level(int level)
{
	struct vreg *vreg;
	vreg = vreg_get(0, "gp4");
	return vreg_set_level(vreg,level);
}
...
static int  do_check_mic(void)
{
	...
	uint8_t   nval1 , nval2;
	int ret_val;
	//set gp4 to 2100 mv to differentiate between omtp, non-omtp headsets
	ret_val = msm_set_vreg_level(2100);
	msm_pm_mic_en(1);
	...
	printk(KERN_INFO "GPIO are %d -  %d   :  %d \n",ret_val, val1,val2 );
	msm_set_vreg_level(3050);
	msleep_interruptible(HEADSET_MIC_BIAS_DELAY);
	nval1 =gpio_get_value(HEADSET_SEND_END_0_GPIO);
	nval2 =gpio_get_value(HEADSET_SEND_END_1_GPIO);
	printk(KERN_INFO "New GPIO are %d -  %d   :  %d \n",ret_val, nval1,nval2 );

	// reset gp4 to 2850 mv for the mic to work normally
	msm_set_vreg_level(2850);
	if( (val1 == 1) && (val2 == 0 ) )
	{
		printk(KERN_INFO "Omtp headset\n");
	...
	else if( (val1 == 1) && (val2 == 1 ) )
	{
		if(nval2 == 1)
                {
			printk(KERN_INFO "stereo headset\n");
		}
		else
		{
			// we should disable mic in this case also to avaoid
			// false send/end detections
			printk(KERN_INFO "non omtp headset\n");
		}
	}
