...
#if defined(CONFIG_TOUCHSCREEN_QUANTUM_OBP) && !defined(CONFIG_TOUCHSCREEN_QUANTUM_OBP_OLD)
#include <linux/qtouch_obp_ts.h>
#endif /* CONFIG_TOUCHSCREEN_QUANTUM_OBP  && !CONFIG_TOUCHSCREEN_QUANTUM_OBP_OLD */
...
#if defined(CONFIG_TOUCHSCREEN_QUANTUM_OBP) && !defined(CONFIG_TOUCHSCREEN_QUANTUM_OBP_OLD)
#define TOUCH_RST_N   84
#define TOUCH_INT_N   41
#define QTOUCH_I2C_ADDR   0x11
#endif /* CONFIG_TOUCHSCREEN_QUANTUM_OBP && !CONFIG_TOUCHSCREEN_QUANTUM_OBP_OLD */
...
static void msm_fb_mddi_power_save(int on)
{
  ...
  if (on) 
  {
      // BUG41884 - commenting out the vreg enable/disable as 
      // a particular rev of the TMD display has trouble coming
      // back out of sleep.  Leaving vreg enabled as there is a negligible 
      // impact on sleep current drain.
...
#if defined(CONFIG_TOUCHSCREEN_QUANTUM_OBP_OLD)
...
#endif /* CONFIG_TOUCHSCREEN_QUANTUM_OBP_OLD */
...
static struct qtouch_ts_platform_data qtouch_platform_data = {
   ...
   .power_cfg  = { /* T7 Gen PowerConfig */
      .idle_acq_int     = 0x12,
      .active_acq_int   = 0x14,
      .active_idle_to	= 0x03,
   },
   .acquire_cfg   = { /* T8 Gen AcquireConfig */
      .charge_time   = 0x08,
      .atouch_drift  = 0x05,
      .touch_drift   = 0x05,
      .drift_susp    = 0x05,
      .touch_autocal = 0x96, /* 0x00 */
      .sync          = 0x01,
   },
   .multi_touch_cfg  = { /* T9 Touch MultiTouchScreen */
      .ctrl    = 0x0b,
      .x_origin   = 0x00,
      .y_origin   = 0x00,
      .x_size     = 0x0c,
   ...
   .noise1_suppression_cfg = { /* T36 */
      .ctrl		= 0x01,
      .reserved = 0x01,
      .atchthr = 0xef,
      .duty_cycle	= 0x08,
#ifndef USE_T36_ALGORITHM
      .drift_thr = 0x00,
      .clamp_thr = 0x00,
      .diff_thr = 0x00,
      .adjustment = 0x00,
      ...
      .offset = {
         ...
         0x0000, 0x0000, 0x0000, 0x0000, 0x0000, /* 15 */ 
         0x0000, 0x0000, 0x0000, 0x0000, 0x0000, /* 20 */
         0x0000, 0x0000, 0x0000, 0x0000, 0x0000, /* 25 */
         0x0000, 0x0000, 0x0000, 0x0000, 0x0000, /* 30 */
         0x0000, 0x0000, 0x0000, 0x0000, 0x0000, /* 35 */
         0x0000, 0x0000, 0x0000, 0x0000, 0x0000, /* 40 */
         0x0000, 0x0000, 0x0000, 0x0000, 0x0000, /* 45 */
         0x0000, 0x0000, 0x0000, 0x0000, 0x0000, /* 50 */
         0x0000, 0x0000, 0x0000, 0x0000, 0x0000, /* 55 */
         0x0000, 0x0000, 0x0000, 0x0000, 0x0000, /* 60 */
         0x0000, 0x0000, 0x0000, 0x0000, 0x0000, /* 65 */
         0x0000, 0x0000, 0x0000, 0x0000, 0x0000, /* 70 */
         0x0000, 0x0000, 0x0000, 0x0000, 0x0000, /* 75 */
         0x0000, 0x0000, 0x0000, 0x0000, 0x0000, /* 80 */
...
static struct i2c_board_info i2c_morrison_devices[] = {
	...
	.platform_data = &mot_key08_device
      ...
      {
	/* camera device for BAM module[keypad/interconnect pcb] */
	I2C_BOARD_INFO("mt9p012", 0x6c >> 1),
      },
      { /* camera device for VCM module[keypad/interconnect pcb] */ 
        I2C_BOARD_INFO("mt9p012_vcm", 0x6C >> 2),
      },
};
...
static struct i2c_board_info i2c_motus_devices[] = {
    ...
    {    /* camera device for BAM module */
        I2C_BOARD_INFO("mt9p012", 0x6C >> 1),
    },
    {   /* camera device for VCM module */
        I2C_BOARD_INFO("mt9p012_vcm", 0x6C >> 2),
    },
};
#endif /* CONFIG_TOUCHSCREEN_QUANTUM_OBP_OLD */
...
#else /* CONFIG_TOUCHSCREEN_QUANTUM_OBP_OLD */
    {    /* touchscreen */
...
// 2 PATH : Add, SND(HEADSET_AND_SPEAKER, 26)
...
static uint32_t camera_on_gpio_table[] = {
   ...
   GPIO_CFG(15, 1, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA), /* MCLK */
};
...
int af_actuator_addr_mt9p012_bam[] = {0x45,0x05 } ;
int af_actuator_addr_mt9p012_vcm[] = {0x0C};
...
static struct msm_camera_sensor_info msm_camera_sensor[] = {
    {
        ...
        .sensor_name	= "mt9p012", /* BAM module */
        .flash_type	= MSM_CAMERA_FLASH_LED,
	.sensor_probe   = MSM_PROBE_INIT(mt9p012),
        .actuator_probe_needed = 1,
        .num_af_actuators = 2,
        .act_addr         = af_actuator_addr_mt9p012_bam,
    },
    {
        .sensor_reset = CAM_RESET_N_SIGNAL,
        .sensor_pwd   = CAM_PWD_SIGNAL,
        .vcm_pwd      = 0, /* w16959 - AF doesn't have separate pwr */
        .sensor_name  = "mt9p012_vcm", /* VCM module */
        .flash_type   = MSM_CAMERA_FLASH_LED,
        .sensor_probe = MSM_PROBE_INIT(mt9p012_vcm),
        .actuator_probe_needed = 1,
        .num_af_actuators = 1,
        .act_addr         = af_actuator_addr_mt9p012_vcm,
    },
...
void zeppelin_setup(void)
{
    ...
    /* // Bug 12252 : Donot Change the GPIO config for 20,94 on zeppelin.
       // This reduces standby current drain by 1.6mA
       // These are unused GPIOs and no need to configure them differently
    ...
    */
    ...
    if(!model_set) {
        msm_hsusb_pdata.product_name = "MB501";    
#ifdef CONFIG_USB_FUNCTION_MASS_STORAGE
        mass_storage_pdata.product = "MB501";
#endif
    }
