...
#define TS_FW_CRASH_ISSUE
#ifdef TS_FW_CRASH_ISSUE
...
static uint32_t qtouch_reset_cnt;
static uint32_t qtouch_resume_cal;
static uint32_t qtouch_num_touch_cnt;
...
static int qtouch_force_calibration(struct qtouch_ts_data *ts)
{
	...
	qtouch_resume_cal = 0;
#ifdef DEBUG_ON
	pr_info("%s: [Disable] qtouch_resume_cal=%d, qtouch_num_touch_cnt=%d\n", __func__, qtouch_resume_cal, qtouch_num_touch_cnt);
#endif
	qtouch_num_touch_cnt = 0;
	return ret;
}
			   ...
			   void *_msg)
{
	...
	if (msg->status & QTM_CMD_PROC_STATUS_CAL) {
		...
		qtouch_num_touch_cnt = 0;
#ifdef DEBUG_ON
		pr_info("%s: [Disable] qtouch_resume_cal=%d\n", __func__, qtouch_resume_cal);
#endif
			      ...
			      void *_msg)
{
	...
	if (qtouch_resume_cal) {
		if (msg->status&0x40) {
			qtouch_num_touch_cnt++;
#ifdef DEBUG_ON
			pr_info("%s: [Increase] stat=%02x, f=%d qtouch_num_touch_cnt=%d\n", __func__,
				msg->status, finger, qtouch_num_touch_cnt);
#endif
		}
		if (msg->status&0x20) {
				qtouch_num_touch_cnt--;
			if (qtouch_num_touch_cnt != 0) {
#ifdef DEBUG_ON
				pr_info("%s: [Decrease] stat=%02x, f=%d qtouch_num_touch_cnt=%d\n", __func__,
					msg->status, finger, qtouch_num_touch_cnt);
#endif
			} else {
				/* Forcing calibration */
...
#if 0 /* Do not need this code for MSM chip */
...
#ifdef TS_FW_CRASH_ISSUE
static void force_bootloader_mode(struct qtouch_ts_data *ts)
{
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (err == 0) {
		...
		if (err) {
			...
			qtouch_force_reset(ts, 0);
			err = qtouch_process_info_block(ts);
			pr_info("%s: force_bootloader_mode = %i\n", __func__, err);
			force_bootloader_mode(ts);
			ts->org_i2c_addr = ts->client->addr;
			ts->client->addr = ts->pdata->boot_i2c_addr;
			err = qtouch_read(ts, &boot_info, 1);
		}
...
static int qtouch_ts_resume(struct i2c_client *client)
{
	...
	if(qtouch_reset_cnt > 100) {
		...
		qtouch_reset_cnt++;
#ifdef DEBUG_ON
		pr_info("%s: [Enable] qtouch_resume_cal=%d, qtouch_num_touch_cnt=%d\n", __func__, qtouch_resume_cal, qtouch_num_touch_cnt);
#endif
		qtouch_num_touch_cnt = 0;
		/* reset the address pointer */
		ret = qtouch_set_addr(ts, ts->obj_tbl[QTM_OBJ_GEN_MSG_PROC].entry.addr);
