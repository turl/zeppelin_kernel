...
static int force_bootloader_mode(void);
static int do_hw_reset(struct i2c_client *client);
#ifdef CONFIG_HAS_EARLYSUSPEND
...
static unsigned char saveConfig = 0;
...
{
	unsigned char value[KEY08_MAX_I2C_WRITE + KEY08_REG_ADDR_SIZE];
	int retval = 0;
	int i = 10;
...
// also look in touchpad.c in vendor/mot/common/touchpad if changing tcal location.
#define TCAL_FNAME	"/data/tcal"
#define MAX_TCAL_PTS	256
#define ZEPPELIN_TCAL_PTS 84

#define TCAL_PTS	ZEPPELIN_TCAL_PTS

// if there is a change to the NV structures corresponding to  NV_MOT_TOUCH_CAL_HEADER_I ,
// NV_MOT_TOUCH_CAL_PTS_0_I .. NV_MOT_TOUCH_CAL_PTS_3_I this tcal_data must reflect
// that change.
typedef struct 
{
  ...
  unsigned short	pts[MAX_TCAL_PTS];
} tcal_data;


static int qtm_obp_load_calibration_data(struct i2c_client *client, int  tcal_file)
{
  static int done_once = 0;
  unsigned int T36_base_addr		= qtm_obp_obj_table_elem[OBJ_ID_TOUCH_CAL_T36].start_pos;
  unsigned int T36_average_offset	= T36_base_addr + 8; // ctrl, version, atchthr, dutycycle, driftthr, clampthr, diffthr, adjustment preceed average, which are 1 byte each.
  unsigned int T36_first_channel_offset	= T36_average_offset + 3; // avg is 2 bytes and temp is 1 byte.
  unsigned int T36_cur_channel_offset = T36_first_channel_offset;
  tcal_data data_read;
  unsigned char data_to_write[KEY08_MAX_I2C_WRITE];
  unsigned int bytes_to_flush_to_ic = 0;
  int i;
  unsigned int T9_base_addr = qtm_obp_obj_table_elem[OBJ_ID_TOUCH_MULTITOUCHSCREEN_T9].start_pos;
  unsigned int T9_burst_len_addr = T9_base_addr + 6;
  
  if( done_once == 1) {
    KEY08_PRINTK("%s: tcal - done once already\n", __FUNCTION__);
    return (TRUE);
  }
  KEY08_PRINTK("%s: tcal - T36_base_addr = 0x%x\n", __FUNCTION__, T36_base_addr);
  if( (tcal_file >= 0 ) && 
      ( sizeof(tcal_data) == sys_read(tcal_file, (void *)&data_read, sizeof(tcal_data) ) ) && 
      ( data_read.num_pts == TCAL_PTS ) )
  {

    if(data_read.burst_len == (unsigned short )qtm_obp_ts_read_register( client , T9_burst_len_addr ) )
    {
	bytes_to_flush_to_ic = 0;
	data_to_write[bytes_to_flush_to_ic++] = (unsigned char)(data_read.average & 0xff); // LSB
	KEY08_PRINTK("%s: tcal - average LSB is 0x%x\n", __FUNCTION__, data_to_write[0]);
	data_to_write[bytes_to_flush_to_ic++] = (unsigned char)( (data_read.average >> 8 ) & 0xff ); // MSB
	KEY08_PRINTK("%s: tcal - average MSB is 0x%x\n", __FUNCTION__, data_to_write[1]);
	data_to_write[bytes_to_flush_to_ic++] = (unsigned char)(data_read.temperature & 0xff);
	qtm_obp_reg_write( client, T36_average_offset, bytes_to_flush_to_ic, data_to_write);

	bytes_to_flush_to_ic = 0;
	for(i = 0; i < TCAL_PTS; i++)
	{
		data_to_write[bytes_to_flush_to_ic++] = (unsigned char)(data_read.pts[i] & 0xff); // LSB
		data_to_write[bytes_to_flush_to_ic++] = (unsigned char)( (data_read.pts[i] >> 8 ) & 0xff ); // MSB
		if( KEY08_MAX_I2C_WRITE == bytes_to_flush_to_ic )
		{
			qtm_obp_reg_write( client, T36_cur_channel_offset, bytes_to_flush_to_ic, data_to_write);
			T36_cur_channel_offset += bytes_to_flush_to_ic;
			bytes_to_flush_to_ic = 0;
		}
	}
	if( bytes_to_flush_to_ic != 0 )
		qtm_obp_reg_write( client, T36_cur_channel_offset, bytes_to_flush_to_ic, data_to_write);
	done_once = 1;
	return (TRUE);
      }
    else{
      KEY08_PRINTK("%s: tcal - burst length mismatch reading tcal\n", __FUNCTION__);
      return (FALSE);
    }
  }
  else{
    KEY08_PRINTK("%s: tcal - error reading tcal\n", __FUNCTION__);
    return (FALSE);
  }
}
...
static int qtm_obp_calibrate(struct i2c_client *client)
{
  ...
  int tcal_file = sys_open(TCAL_FNAME, O_RDONLY, 0);
  ...
  if ( qtmTsGl->mode == KEY08_MODE_OBP)
  {	
      if( (tcal_file >= 0) && (TRUE == qtm_obp_load_calibration_data( client , tcal_file)) )
      {
          regValue = 0xDB; 
          KEY08_PRINTK("%s: tcal successful, issuing 0xDB instead of 0x1 \n", __FUNCTION__);
      }
      
      ret = qtm_obp_reg_write (client,
                               qtm_obp_obj_table_elem[OBJ_ID_GEN_COMMANDPROCESSOR_T6].start_pos+QTM_OBP_T6_CALIBRATE, 
                               sizeof(regValue), 
                               (char *)&regValue);
  }
  clk_busy_wait(6000);
...
static void qtm_obp_ts_work_func(struct work_struct *work)
{
	...
	else if ( qtmTsGl->mode == KEY08_MODE_OBP )
	{
	       ...
	       && buf[QTM_OBP_REPORT_ID_BYTE] <= qtm_obp_obj_table_elem[OBJ_ID_GEN_COMMANDPROCESSOR_T6].report_id_high )
	  {
		...
		{
		  calibrating = 1;
		  printk("qtm_obp_obp_ts_work_func: The IC is calibrating.\n");
		}
	    else if ((calibrating != 0) && 
		       ((buf[QTM_OBP_T6_STATUS] & QTM_OBP_T6_STATUS_CAL) == 0))
		{
		  calibrating = 0;
		  printk("qtm_obp_obp_ts_work_func: The IC is done calibrating.\n");
		  clk_busy_wait(3000);
		}
	      else if (((buf[QTM_OBP_T6_STATUS] & QTM_OBP_T6_STATUS_CAL) == 0))
                {
                  if (saveConfig)
                     saveConfig = 0;
		  printk("qtm_obp_obp_ts_work_func: The IC Saved CONFIG.\n");
                  memset (qtmTsGl->chkSum, 0, 6);
                  for (i=0;i<ret;i++)
                  {
                      qtmTsGl->chkSum[i] = buf[i];
                  }
                  for (i=0;i<ret;i++)
                  KEY08_PRINTK("buf[%d] = 0x%x\n", i, qtmTsGl->chkSum[i]);
                } 
 ...
 */
    ...
    {
        // As the IC touch will suffer a reset, just continue with normal operation after the reset.
        needs_reading = 1;

		clk_busy_wait(3000);

        // Force bootloader mode
        ret = force_bootloader_mode();
        if( ret < 0 )
        {
            printk("qtm_obp_ts_probe: qtm_obp_ioctl (set bootloader mode) error %d\n",ret);
            return ret;
        }
        else
        {
            printk("qtm_obp_ts_probe: qtm_obp_ioctl (set bootloader mode) done ok!\n");
        }

        // Reset the IC touch (send 0x00 0x00 to bootloader address)
        client->addr = qtm_obp_bl_addr;
        ...
        {
            printk("qtm_obp_ts_probe: i2c_master_recv (bootloader client) done ok!\n");
            qtmTsGl->mode = KEY08_MODE_BOOTLOADER;
            
            printk("qtm_obp_ts_probe: Sending Reset command (0x00,0x00)\n");
            data[0] = 0x00;
            data[1] = 0x00;
            ret = i2c_master_send(client,data,2);
        }

        client->addr = qtm_obp_normal_addr;
        clk_busy_wait(500000);
    }
	...
	do
	{
	    ...
	    if( i2c_master_recv(client,data,6) < 0 )
		{
			printk("qtm_obp_ts_probe: i2c_master_recv (normal client) error  %d\n",ret);
			client->addr = qtm_obp_bl_addr;
			if ((ret = i2c_master_recv(client, data, 1 )) < 0 )
			{
                // Perform a HW reset for UNKNOWN mode of operation.
                if( !reset_done )
                {
                    reset_done = 1;
                    ret = do_hw_reset(client);
                    if( ret < 0 )
                    {
                        printk("qtm_obp_ts_probe: do_hw_reset() error %d\n",ret);
                    }
                    else
                    {
                        printk("qtm_obp_ts_probe: do_hw_reset() ok\n");
                    }
                }
        ...
        {
            // Perform a HW reset for NORMAL mode of operation.
            if( !reset_done )
            {
                reset_done = 1;
                ret = do_hw_reset(client);
	...
	while (retry-- > 0 && qtmTsGl->mode == KEY08_MODE_NORMAL )
	{
        ...
        // Handle UNKNOWN Mode
	if ((ret < 0) && ( qtmTsGl->mode != KEY08_MODE_UNKNOWN ))
	{
	...
	if ( qtmTsGl->mode == KEY08_MODE_NORMAL )
	{
        ...
        // Comment this line for the SW workaround for the issue 48651
...
static int qtm_obp_ioctl(struct inode *node, struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case KEY08_IOCTL_SET_BOOTLOADER_MODE:
          ...
          if ( (qtmTsGl->mode == KEY08_MODE_OBP) || (qtmTsGl->mode == KEY08_MODE_UNKNOWN))
          {
	...
	case KEY08_IOCTL_SAVE_CONFIG_STATUS:
		rc = saveConfig;
		break;
	...
	case KEY08_IOCTL_CALIBRATE:
		...
		qtm_obp_calibrate(qtmTsGl->client);
            }
            else
            {
                calibrating = 0;
	...
	case KEY08_IOCTL_GET_CHECKSUM: 
             chkSum = (char *) arg;
             memset (chkSum, 0 , 6);
             for (i = 0; i < 6; i++)
             {
                *chkSum = qtmTsGl->chkSum[i];
                chkSum++;
             }
             rc = 0;
             break;
...
static int qtm_obp_get_touch_sens_data (char * dataPtr)
{
    ...
    // Disable Touch Process
    printk("%s: Disable Touch Screen Process \n", __FUNCTION__);
    ret = qtm_obp_reg_write (qtmTsGl->client, qtm_obp_obj_table_elem[OBJ_ID_GEN_POWERCONFIG_T7].start_pos+QTM_OBP_T7_IDLE,
	sizeof(char), (char *)&regValue);
	clk_busy_wait(3000);
    ...
    printk("%s: End Disable Touch Screen Process \n", __FUNCTION__);
    // END  Disable Touch Process
    ...
    // ENABLE Touch Process
   
    printk("%s: Enable  Touch Screen Process \n", __FUNCTION__);
    ret = qtm_obp_reg_write (qtmTsGl->client, qtm_obp_obj_table_elem[OBJ_ID_GEN_POWERCONFIG_T7].start_pos+QTM_OBP_T7_IDLE,
         sizeof(char), (char *)&lpIdleValueEeprom);
         clk_busy_wait(3000);
    ...
    printk("%s: End Enable  Touch Screen Process \n", __FUNCTION__);
    // END ENABLE Touch Process
...
static int force_bootloader_mode(void)
{
    ...
    printk("force_bootloader_mode: Entering....\n");

    memset(data,0,sizeof(data));

    // set Change pin to LOW
    printk("force_bootloader_mode: set Change pin to LOW....\n");
    gpio_request(KEY08_TS_GPIO, "qtm_obp_ts_gpio");
    gpio_direction_output(KEY08_TS_GPIO, 0);
    gpio_set_value(KEY08_TS_GPIO,0);

    // set RESET to Low
    printk("force_bootloader_mode:  set RESET to LOW....\n");
    gpio_request(KEY08_TS_RESET, "touch_rst_n");
    gpio_set_value(KEY08_TS_RESET,0);
    ...
    // set RESET to high
    printk("force_bootloader_mode:  set RESET to HIGH....\n");
    gpio_set_value(KEY08_TS_RESET ,1);
    ...
    // set Change pin to high
    printk("force_bootloader_mode: set Change pin to HIGH....\n");
    gpio_set_value(KEY08_TS_GPIO ,1);

    // set Change pin back to input for interrupt
    printk("force_bootloader_mode: set Change pin to INPUT....\n");
    gpio_direction_input(KEY08_TS_GPIO);
    ...
    {
        rc = i2c_master_recv(qtmTsGl->client, data, 1);
        printk("force_bootloader_mode: reading BL status = 0x%02X\n",data[0]);
        clk_busy_wait(3000);
        loopCount++;
    }
    ...
    if (loopCount >=100)
    {
        printk("force_bootloader_mode: unable to set to BL mode\n");
        qtmTsGl->client->addr = qtm_obp_normal_addr;
        rc = -1;
    }
    else
    {
        // we are bootloader mode
        qtmTsGl->mode = KEY08_MODE_BOOTLOADER;
        printk("force_bootloader_mode: We are in BL mode, got 0x%02X\n",data[0]);

        if ( (data[0] & 0xF0 ) == 0xC0 )
        {
            printk("force_bootloader_mode: send unlock code\n");
            ...
            {
                // Something wrong, Bootloader is not ready for receiving data 
                printk("force_bootloader_mode: Something wrong, Bootloader is not ready for receiving data \n");
                rc = -1;
            }
            else
            {
        ...
        {
            printk("force_bootloader_mode: Already in BOOTLOADER mode\n");
            rc = KEY08_MODE_BOOTLOADER;
        }
    }
    ...
    printk("force_bootloader_mode: Exiting....\n");
