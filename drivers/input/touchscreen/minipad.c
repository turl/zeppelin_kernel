...
#define MINIPAD_GET_TRACKBALL_REL_EVT(evt) ((evt >= 0) ? +1 : -1) 
/******************************************************************************
                                      ...
                                      unsigned int y, unsigned int touch)
{
    ...
    // Range check
    if( !( (x >= MINIPAD_MOUSE_ZEPPELIN_X_RANGE_MIN && x <= MINIPAD_MOUSE_ZEPPELIN_X_RANGE_MAX) && 
           (y >= MINIPAD_MOUSE_ZEPPELIN_Y_RANGE_MIN && y <= MINIPAD_MOUSE_ZEPPELIN_Y_RANGE_MAX) ) )
    {
        minipad_log("mouse: <<< Out of range: x=%4i, y=%4i\n", x, y);
        /* If we see a lift off then reset the variables */
        if(touch == 0)
        {
    ...
    if(((x_start == 0) && (y_start == 0)) && (touch == 1))
    {
        x_start = x;
        y_start = y;
        minipad_log("mouse: <<< Touchdown: x=%4i, y=%4i\n", x, y);
        return;
    }
    ...
    minipad_log("mouse: x_th:%d\t y_th:%d\t x_scale:%d\t y_scale:%d\n", 
    ts->logic_params.mouse_th_x, ts->logic_params.mouse_th_y, 
    ts->logic_params.mouse_scale_x, ts->logic_params.mouse_scale_y);
    
    minipad_log("mouse: x_start:%4i\t y_start:%4i\t x:%4i\t y:%4i\t REL_X:%4i\t REL_Y:%4i\n",
    x_start, y_start, x, y, x_rel, y_rel);

    // Scale the values
    x_scaled_rel = ( x_rel * ts->logic_params.mouse_scale_x )/100;
    y_scaled_rel = ( y_rel * ts->logic_params.mouse_scale_y )/100;

    // Send relative X and Y        
    if( (abs(x_scaled_rel) > ts->logic_params.mouse_th_x) && 
        (abs(y_scaled_rel) > ts->logic_params.mouse_th_y) )
    {
        minipad_log("mouse: XY meet Mouse threshold met REL_X=%i REL_Y=%i REL_S_X=%i "\
        "REL_S_Y=%i positions changed\n", x_rel, y_rel, x_scaled_rel, y_scaled_rel);

        for( evt_loop_diag = 0; 
             ( evt_loop_diag < (abs(x_scaled_rel)/ts->logic_params.mouse_th_x) &&
               evt_loop_diag < (abs(y_scaled_rel)/ts->logic_params.mouse_th_y) ); 
             evt_loop_diag++ )
        {
            input_report_rel(ts->input_dev, REL_X, MINIPAD_GET_TRACKBALL_REL_EVT(x_scaled_rel));
            input_report_rel(ts->input_dev, REL_Y, MINIPAD_GET_TRACKBALL_REL_EVT(y_scaled_rel));
        }
        input_sync(ts->input_dev);        

        for(evt_loop = evt_loop_diag; evt_loop < (abs(x_scaled_rel)/ts->logic_params.mouse_th_x); 
            evt_loop++)
        {
            input_report_rel(ts->input_dev, REL_X, MINIPAD_GET_TRACKBALL_REL_EVT(x_scaled_rel));
            input_sync(ts->input_dev);
        ...
        for(evt_loop = evt_loop_diag; evt_loop < (abs(y_scaled_rel)/ts->logic_params.mouse_th_y); 
            evt_loop++)
        {
            input_report_rel(ts->input_dev, REL_Y, MINIPAD_GET_TRACKBALL_REL_EVT(y_scaled_rel));
            input_sync(ts->input_dev);
        ...
        x_start = x - (x_scaled_rel % ts->logic_params.mouse_th_x);
        y_start = y - (y_scaled_rel % ts->logic_params.mouse_th_y);
    }
    else if(abs(x_scaled_rel) > ts->logic_params.mouse_th_x)
    {
        ...
        for(evt_loop = 0; evt_loop < (abs(x_scaled_rel)/ts->logic_params.mouse_th_x); evt_loop++)
        {
            input_report_rel(ts->input_dev, REL_X, MINIPAD_GET_TRACKBALL_REL_EVT(x_scaled_rel));
            input_sync(ts->input_dev);
        }
    ...
    else if(abs(y_scaled_rel) > ts->logic_params.mouse_th_y)
    {
        ...
        for(evt_loop = 0; evt_loop < (abs(y_scaled_rel)/ts->logic_params.mouse_th_y); evt_loop++)
        {
            input_report_rel(ts->input_dev, REL_Y, MINIPAD_GET_TRACKBALL_REL_EVT(y_scaled_rel));
            input_sync(ts->input_dev);
        }
    ...
    {
         minipad_log("mouse: NO threshold met REL_X=%i REL_Y=%i REL_S_X=%i REL_S_Y=%i\n", 
         x_rel, y_rel, x_scaled_rel, y_scaled_rel);
    }
...
static void minipad_work_func(struct work_struct *work)
{
    ...
    else
    {
        ...
        case Q5199225K03_10BIT_TSP_MSG:
            ...
            {
                // check garbage data
                if((buf[Q5199225K03_MSG_PTR10_DATA_XH_BYTE] & 0xfc) || 
                   (buf[Q5199225K03_MSG_PTR10_DATA_YH_BYTE] & 0xfc))
                {
...
static int minipad_reg_read(struct i2c_client *client , int reg )
{
    ...
    {
        // Check if it is in BL Mode
        rc = i2c_master_recv(ts->client, buf, 6);
        if (rc > 0)
           rc = buf[2+(reg%4)];
        else
...
static void minipad_logic_params_init(int version)
{
    if(version == 1) //zeppelin
    {
        ...
        ts->logic_params.mouse_scale_x = MINIPAD_MOUSE_ZEPPELIN_X_SCALE;
        ts->logic_params.mouse_scale_y = MINIPAD_MOUSE_ZEPPELIN_Y_SCALE;
    }
    else //motus/default
    {
...
static int minipad_ioctl(struct inode *node, struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case MINIPAD_IOCTL_SET_SCALING:
        bCount = copy_from_user(kernelBuffer, (char *)arg, (unsigned long)sizeof(A_MINIPAD_REG)); 
        if ( bCount == 0 )
        {
            A_MINIPAD_REG_PTR    regV;
            ...
            minipad_log("minipad_ioctl: setting %d scaling\n", (int)regV->reg);

            if((int)regV->reg == 88) //X
            {
                ts->logic_params.mouse_scale_x = (int)regV->value;
                minipad_log("minipad_ioctl: setting X scaling = %d\n", (int)regV->value);
            }
            else if((int)regV->reg == 89) //Y
            {
                ts->logic_params.mouse_scale_y = (int)regV->value;
                minipad_log("minipad_ioctl: setting Y scaling = %d\n", (int)regV->value);
            }
        }
        break;

    case MINIPAD_IOCTL_SET_THRESH_HOLD:
        bCount = copy_from_user(kernelBuffer, (char *)arg, (unsigned long)sizeof(A_MINIPAD_REG)); 
        if ( bCount == 0 )
        {
            A_MINIPAD_REG_PTR    regV;
            ...
            minipad_log("minipad_ioctl: setting %d thresh hold\n", (int)regV->reg);

            if((int)regV->reg == 88) //X
            {
                ts->logic_params.mouse_th_x = (int)regV->value;
                minipad_log("minipad_ioctl: setting X thresh hold = %d\n", (int)regV->value);
            }
            else if((int)regV->reg == 89) //Y
            {
                ts->logic_params.mouse_th_y = (int)regV->value;
                minipad_log("minipad_ioctl: setting Y thresh hold = %d\n", (int)regV->value);
            }
        }
        break;
