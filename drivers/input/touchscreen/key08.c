...
struct key08_ts_data {
	...
	struct miscdevice key08_misc_device;
...
int key08_disable(struct key08_ts_data *ts)
{
	int err;
	uint8_t lp_mode;

	KEY08_PRINTK("%s: Enter: enabled=%u\n", __func__, ts->enabled);
	if (ts->mode != KEY08_MODE_NORMAL) {
		err = 0;
		goto err_not_normal_mode;
	}

	mutex_lock(&ts->lock);
	if (ts->enabled > 0) {
		/* put device in low-power mode to prevent any further interrupts */
		lp_mode = 0x00; 
		err = key08_reg_write(ts, Q51001211009AK08_LP_MODE_ADDR,
				sizeof(lp_mode), &lp_mode);
		if (err) {
			pr_err("%s: Writing lp mode failed\n", __func__);
			err = -EIO;
			goto err_write_lp_mode;
		}
		clk_busy_wait(3000);
		clk_busy_wait(3000);

		/* disable interrupts and cancel any pending work */
		key08_disable_irq(ts);
		err = cancel_work_sync(&ts->work);
		if (err) {
			pr_err("%s: Unable to cancel pending work\n", __func__);
			err = -EBUSY;
			goto err_cancel_work;
		}

		/* work function could have re-enabled before it was cancelled */
		key08_disable_irq(ts);

		/* clean-up TSB timer and state */
		hrtimer_cancel(&ts->tsbTimer);
		ts->tsbState = KEY08_TSB_UNTOUCHED;
	...
	KEY08_PRINTK("%s: Exit: enabled=%u\n", __func__, ts->enabled);
	return 0;

err_power_off:
err_cancel_work:
	/* return chip to running state */
	lp_mode = configRegs[Q51001211009AK08_LP_MODE_ADDR];
	key08_reg_write(ts, Q51001211009AK08_LP_MODE_ADDR,
		sizeof(lp_mode), &lp_mode);
	clk_busy_wait(3000);
	clk_busy_wait(3000);
	key08_calibrate(ts);
...
err_write_lp_mode:
	mutex_unlock(&ts->lock);
err_not_normal_mode:
	KEY08_PRINTK("%s: Exit(%d): enabled=%u\n", __func__, err, ts->enabled);
	return err;
}

int key08_enable(struct key08_ts_data *ts)
{
#ifdef CONFIG_MACH_MOT
	uint8_t buf[15];
	...
	if (ts->enabled < 1) {
		...
		err = key08_init_panel(ts);
		if (err < 0) {
			pr_err("%s: Panel init failed\n", __func__);
			err = -EIO;
			goto err_ts_init;	
		}
		...
		do {
			buf[0] = 0xff;
			err = i2c_master_recv(ts->client, buf, bytes2Read);
			if (err > 0) {
				clk_busy_wait(3000);
...
/* try setting the interrupt low. This way if data is missed, the ISR will still be executed */
	...
	*/
		...
		mutex_init(&tsGl->lock);
		tsGl->enabled = 1;

		if (client->irq) 
		{
			spin_lock_init(&tsGl->irq_lock);
			tsGl->irq_count = 1;
			tsGl->use_irq = TRUE;
	...
	tsGl->key08_fops.owner	= THIS_MODULE;
	tsGl->key08_fops.open	= key08_open;
	tsGl->key08_fops.ioctl	= key08_ioctl;
	tsGl->key08_fops.write	= key08_write;
	tsGl->key08_misc_device.minor	= MISC_DYNAMIC_MINOR;
	tsGl->key08_misc_device.name	= KEY08_PF_NAME;
	tsGl->key08_misc_device.fops	= &tsGl->key08_fops;
	ret = misc_register(&tsGl->key08_misc_device);
	if (ret != 0) {
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void key08_ts_early_suspend(struct early_suspend *handler)
{
	struct key08_ts_data *ts;

	KEY08_PRINTK("%s: Entering\n", __func__);
	ts = container_of(handler, struct key08_ts_data, early_suspend);
	key08_ts_suspend(ts->client, PMSG_SUSPEND);
	KEY08_PRINTK("%s: Exiting\n", __func__);
}

static void key08_ts_late_resume(struct early_suspend *handler)
{
	struct key08_ts_data *ts;

	KEY08_PRINTK("%s: Entering\n", __func__);
	ts = container_of(handler, struct key08_ts_data, early_suspend);
	key08_ts_resume(ts->client);
	KEY08_PRINTK("%s: Exiting\n", __func__);
}
#endif
...
static int key08_ioctl(struct inode *node, struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	ts = container_of(filp->f_op, struct key08_ts_data, key08_fops);
	if (!ts)
		return -EINVAL;
