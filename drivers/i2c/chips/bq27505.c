...
static int curr_battery_capacity = 0;
...
{
#if 1
	/* w21558, In case of low battery(), 
	we should check battery temp more often */
	bq27505_update_battery();
	if (curr_battery_capacity <= LOW_BATTERY_CAPACITY) {
		temperature_timer.expires = jiffies + 5*HZ;
	}
	else {
...
static bool battery_capacity(int *capacity)
{
	...
	/* w21558, current battery capacity is saved */	
	curr_battery_capacity = *capacity;
	return ret;
}
